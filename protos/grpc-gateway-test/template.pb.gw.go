...
func request_Enroll_SendGet_1(ctx context.Context, marshaler runtime.Marshaler, client EnrollClient, req *http.Request, pathParams map[string]string) (proto.Message, runtime.ServerMetadata, error) {
	...
	val, ok = pathParams["mail"]
	if !ok {
		return nil, metadata, status.Errorf(codes.InvalidArgument, "missing parameter %s", "mail")
	}

	protoReq.Mail, err = runtime.String(val)

	if err != nil {
		return nil, metadata, status.Errorf(codes.InvalidArgument, "type mismatch, parameter: %s, error: %v", "mail", err)
	}
